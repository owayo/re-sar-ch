...
既定の `text` は要約です。エピソードは**見出しになった系列ごとにまとめて**出します。
同じ系列が 1 日を通じて散発的に鳴ると、その回数だけエピソードができるためで、
実際のファイルでは 23 系列に対して 111 エピソードが立ち、1 件ずつ並べると
「何が鳴っているか」が 1400 行に薄まりました。系列ごとに、指標名・件数・最高優先度を
先に出し、その下に各エピソードの時刻を並べます。

**報告の義務は要約でも果たします。** 各まとまりには時刻・回ごとの優先度・採取回数が
残り、**同時に別の系列が鳴ったエピソードは `+N 系列`** と出ます (その重なりが
「何かが起きた」手がかりになるため)。比較基準の出所・評価の網羅度・系列に固有の留保も
そのままです。「確かめていないこと」はエピソード本文から外しましたが、
エピソードごとではなく**指標ごとに 1 度**、報告の末尾で必ず出します。

省くのは**繰り返しだけ**で、検出ごとの内訳 (観測値の列挙・比較基準の数値・窓の幅) と、
...
構造体の配置を「実行ホスト」ではなく「**ファイルを書いたホストの ABI**」に合わせて解決するため、
...
SAR_MATRIX_OUTPUT=target/sar-matrix/<実行ディレクトリ> SAR_MATRIX_RESUME=1 make sar-upstream-all
```
...
測定表の `activity_decode=complete` は、宣言された全activityの計画作成・全サンプルのデコードが成功し、
読み飛ばしが0件だったことを表します。元ファイルに無いフィールドや単位不明の値は取得不可のままです。
`exact=true` は末尾一致を表し、歴史的な `sar` との全文一致を意味しません。テキスト比較は測定表の別列です。
...
以下の2表の **`exact=true` は、途中終了や不完全なレコードがなく、走査終了位置が
ファイルサイズと一致した**ことを表します。全項目のデコードや、本家 `sar` との計算値・
テキストの一致を意味するものではありません。それらの対応範囲と比較結果は別途記載します。
...
CentOS 3.9／4.9／5.11 を含む全11ケースで、reSARch の読み取りと末尾までの走査を確認済みです。
