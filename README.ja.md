...
既定の `text` は要約です。エピソードごとに見出し・優先度・根拠の充足度・関わった系列と、
検出 1 件につき 1 行の根拠を出します。省くのは**繰り返しだけ**で、検出ごとの内訳
(観測値の列挙・比較基準の数値・窓の幅) と、検出パターンが決まれば中身も決まる
「考えられる解釈」がそれにあたります。`--verbose` を付けると以前と同じ全文に戻ります。

**報告の義務は要約でも果たします。** 比較基準の出所・評価の網羅度・系列に固有の留保は
そのまま出ます。「確かめていないこと」はエピソード本文から外しましたが、
