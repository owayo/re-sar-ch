...
└───────────────────────────────────────────────────────────────────────────────────┘
←→ activity  ↑↓ 時刻  i item (3)  / 絞り込み  g/G 先頭末尾  c/[] 列  v グラフ  ? help  q 終了
```
...
- **画面幅に入らない列は潰さずに落とします。** 外した列数はタイトルに
  「他 15 列 (C で選ぶ)」と出るので、消えたわけではないと分かります。

`C` を押すと列の一覧が出て、`Space` で出し入れ、`a` で全列と既定の切り替え、
`Enter` で確定します。値が出なかった列も一覧には並び、「(全時刻で値なし)」と注記が付きます。
列幅は全サンプルの値を見て決めるので、行をスクロールしても幅が揺れません。

グラフは表の上に出て、選んでいる activity / item / 列の 1 系列を描きます。
表で選んでいる時刻には縦線が立つので、表とグラフが同じ時点を指していることが分かります。
