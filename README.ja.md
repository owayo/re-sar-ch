...
el7 の `sa2sar` 出力を保存済みの `sar` テキストとバイト単位で比べるときは、日付の設定を揃えてください。保存済みテキストの先頭行が `YYYY-MM-DD` なら `S_TIME_FORMAT=ISO` を指定します。変わるのは先頭行の日付だけで、統計値は変わりません。
...
収録されている activity がタブになり、選んだ item の時系列を表とグラフで読めます。`?` でキー操作の一覧が出ます。通常画面で `Esc` を押すと終了します。`—` は値が無いことを表し、0 ではありません。欠測のところではグラフの線も切れます。

![メモリ使用状況を時系列グラフと表で表示した TUI](docs/images/tui.png)

画像内の終了キーは変更前の `q` 表記です。現在は `Esc` で終了します。
