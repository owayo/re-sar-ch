...
C ライブラリに一切依存せず単体で解析する Rust 製 CLI である。解析対象は CentOS 3〜5 の sysstat 5.0.5 / 7.0.2 の形式、および sysstat v9 系から
...
CentOS 3／4／5 の `0x2163` / `0x2169` は `format::legacy` でヘッダ・レコード境界を検証する。
固定部に同居する統計を重なりのあるビューとして公開し、`layout::legacy` が現在のフィールド索引へ
直接対応付ける。CPU 集約行と個別行にはそれぞれの配置を使う。実バイト列の中間変換は行わない。
読み取り後は共通の `series` / `output` 経路を使う。制約は [旧形式の仕様](format/07-legacy-centos.md) に記す。
...
`resarch sa2sar <FILE> [-o <OUTPUT>] [--utc] [--no-mmap] [--sar-profile <P>] [--sar-page-size <B>]` は
...
書いた `sarDD` と同じテキストになる (§9.1)。`sa2` がまだ動いていない当日分の
`sarDD` を後から作る、という用途を想定している。
...
値は変えない。この区別を `docs/compatibility.ja.md` (英語版は `compatibility.md`。README からリンクする) で明示する。

**入力ファイルの世代と、再現する出力仕様の世代は別の設定**として管理する。
既定では、どの世代のファイルも現行版 (v12.8.0) の `sar` 書式で出す。
`--sar-profile` を指定すると、そのファイルを書いた側の `sar` の出力を再現する。

| `--sar-profile` | 再現する `sar` | 読める世代 | 対象の入口 |
|---|---|---|---|
| `current` (= `sysstat-12.8.0`、既定) | 本家 v12.8.0 | 全世代 | `sar` / `sa2sar` / `sadf` |
| `sysstat-10.1.5-el7` | RHEL / CentOS 7 の `sysstat-10.1.5-*.el7` | 0x2171 だけ | `sar` / `sa2sar` (テキストだけ) |

設計上の判断は 3 つある。

1. **プロファイルは配布物の単位で名乗る。** 同じ `10.1.5` でも el7 のパッチ
   (`NR_CPUS` 8192、`dyn-tick`、ファイルシステム統計のバックポート) で出力が変わる。
   検証したのは el7 のパッケージなので、名前もそれに合わせ、`10.1.5` のような省略形は
   受け付けない。
2. **ファイルの版から自動で選ばない。** ヘッダの版は `sadc` の版であって、`sa2` が使った
   `sar` の版や配布元のパッチを表さない。`-R` のページサイズもファイルに無い。
   推定で書式と計算を変えると既存の出力が黙って変わるので、常に明示指定にする。
3. **現行版の経路と共有しない。** el7 の計算は `series::el7`、描画は `output::sar_el7`、
   引数は `cli::sar_el7_args` に置き、`series::compute` / `output::sar_text` を
   分岐させない。式・型の幅・演算順序・平均の丸め・描画の流れ (見出しの数え方、
   RESTART / COM 行) が版ごとに違い、共有すると片方の修正がもう片方の出力を黙って変える。
   「出力層で値を計算しない」は el7 経路でも同じで、`series::el7` が値を、
   `output::sar_el7` が `printf` 相当の書式化を受け持つ。

仕様と検証の手順は [`format/05-sysstat-10.1.5-el7.md`](format/05-sysstat-10.1.5-el7.md)。
`sadf` 互換出力に el7 のプロファイルを広げないのは、JSON / XML / CSV のキー・構造・
欠落値の扱いを版ごとに検証していないためで、指定されたら出力する前にエラーにする。
...
`docs/formats.ja.md` (英語版は `formats.md`。README からリンクする) はこの 4 軸の現在の範囲と検証の限界を掲載する。revision ごとの対応は
...
| 段階 | 意味 | 現在の範囲 |
|---|---|---|
| 識別 | どの世代のファイルか判定でき、診断に出せる | `0x015d`、`0x115a` 〜 `0x2175` (登録済みの28形式) |
| ヘッダ解釈 | 採取日時・ホスト・記録されている統計の一覧を読める | 登録済みの全形式 (保存されていないメタデータは診断に明記) |
| レコード走査 | レコード境界を確定でき、末尾まで余りなく読める | 同上 |
| 統計デコード | 各 activity の値を取り出せる | 全形式の対応フィールド。旧配列・単位の制限は [旧形式仕様](format/09-legacy-generations.md) |
...
裏が取れる。公式テスト corpus のない旧形式は、当該リリースを隔離コンテナでビルドして
実採取したファイル、本家描画、独立 fixture を根拠にする。収集対象と結果は
[自動採取仕様](format/06-live-matrix.md) と [旧形式の検証](format/09-legacy-generations.md) に残す。
...
`detect` の所見は日本語と英語で出る。決め方は `model::lang` に集約した。

| # | 材料 | 理由 |
|---|---|---|
| 1 | `--lang ja\|en` | 明示指定が最優先 |
| 2 | `RESARCH_LANG` | アプリ固有の環境変数 |
| 3 | `LC_ALL` → `LC_MESSAGES` → `LANG` | POSIX の優先順。`C` / `POSIX` / `C.UTF-8` は**ここで英語に確定** |
| 4 | `LANGUAGE` | GNU gettext の「メッセージに使いたい言語」。`:` 区切りの先頭一致 |
| 5 | ローカルタイムゾーン | `Asia/Tokyo` (と別名の `Japan`) なら日本語 |
| 6 | — | 既定は英語 |

**ロケール環境変数をタイムゾーンより先に見る。** ロケールは「どの言語で読みたいか」の
宣言そのもので、タイムゾーンは「どこにいるか」でしかない。日本から英語環境で使う人の
`LANG=en_US.UTF-8` を位置で上書きするのは、指示を無視することになる。

**`LC_ALL=C` は `LANGUAGE` より強い。** `C` ロケールは「決定的な英語出力がほしい」という
意思表示で、スクリプトやテストがこれに頼る。ここで `LANGUAGE=ja` に負けるとその決定性が壊れる。

**タイムゾーンは識別子で見る。** オフセットが UTC+9 かどうかで判定すると
韓国 (`Asia/Seoul`) も日本語になる。

##### 翻訳の持ち方

- **静的な文は [`model::Text`] の対で持つ。** 日本語配列と英語配列を別に置くと
  件数と順序がずれ、ずれても型は通る。1 件を 1 つの値にまとめ、両方の言語を同時に
  要求すれば、片方だけ足した時点でコンパイルが止まる
- **数値を埋める文は言語ごとの `format!` を書く。** 保存した書式文字列へ後から
  引数を当てる形にしない (引数の個数・型・順序が静的に保てなくなる)。
  助詞・語順・前置と後置が言語で違うので、**意味のまとまり 1 文をそれぞれの言語で書く**
- **語を連結して文を作らない。** 「`{指標}` が `{方向}` へ離れた」を語ごとの `Text` から
  組み立てると、英語の語順で破綻する
- **英語の数え上げは [`model::count_en`] に通す。** `1 breaks` を出さないためで、
  不規則な複数形 (`stretch` → `stretches`) があるので両方の形を呼び出し側が渡す。
  日本語側では使わない (数による語形変化が無く、助数詞は名詞ごとに決まる)
- **言語は分析層まで届ける。** 文を作るのは分析層なので (§2)、出力層で訳し直すと
  `text` と JSON で違う文が出る。`DetectOptions::lang` から
  `Assessment::lang` へ引き継ぎ、出力層はそれを読む
- **規律 23 (比較演算子と説明文の境界の一致) は両方の言語で守る。**
  `metric_catalog` のテストが `boundary_phrase` の文言を日英それぞれの rationale に
  要求する。片方だけ守っても、もう片方を読む人には境界上の値の扱いが伝わらない

#### 本家 `sar` の環境変数 — 読むのは CLI 層だけ

`model::lang` が**独自出力**の言語を決めるのに対し、`model::sysstat_env` は
**互換出力**の書式を決める。本家 `sar` が書式の一部を環境から取るため、
バイト単位で一致させるにはその決め方も写す必要がある。

| 変数 | 効く先 | 決まるもの |
|---|---|---|
| `S_TIME_FORMAT=ISO` | `sar` テキストのバナー行 | 日付が `%x` → `%Y-%m-%d` |
| `S_REPEAT_HEADER=<N>` | `sar` テキストの列見出し | N 行ごとに再表示 (端末でないときだけ) |

**`sadf` には効かない。** 本家は `sadf` で `S_F_PREFD_TIME_OUTPUT` を立てないので、
日付は常に `%Y-%m-%d`、時刻は常に `%H:%M:%S` になる (03 §6.1 の組み合わせ表)。
「互換出力 6 形式は同じ値を出す」の**値は統計量のこと**であり、
日時の字句表現やヘッダの再表示まで揃えるという意味ではない。

**環境を読むのは CLI 層の 1 箇所だけ。** 出力層へは解決済みの値を
`SarTextOptions::date_format` / `SarTextOptions::header_rows` に載せて渡す。
出力層が `std::env::var` を直接呼ぶと、同じ入力・同じオプションでも出力が変わる
隠れた依存になり、ライブラリとして呼んだときやテストの並行実行で再現しない
(`std::env::set_var` は edition 2024 で `unsafe` でもある)。
判定そのものは環境変数と端末高を引数で受ける純粋関数にしてあり、
テストは環境を書き換えずに全分岐を突ける。

**端末高は「stdout が端末か」を先に確かめてから取る。** 本家の条件は
`ioctl(STDOUT_FILENO, TIOCGWINSZ)` の成否であって「端末がどこかに在るか」ではない。
`/dev/tty` に回る実装をそのまま使うと、パイプへ出しているのに端末の高さで
見出しが増える。

**`HeaderRows` は `Default` が 86400 になる newtype にした。** `u32` を直に持つと
`SarTextOptions` の `#[derive(Default)]` が `0` を入れ、`lines >= rows` が常に真 =
毎サンプル再表示に化ける。既定値が壊れる向きを型で塞いでいる。

**86400 行は「実質再表示しない」ではない。** 本家の `lines` は印字行数ではなく、
ビットマップを持つ activity では `count_bits(cpu_bitmap)` を足す。
`-P ALL` と `-A` は `memset(bitmap, ~0, BITMAP_SIZE(NR_CPUS))` でビットマップを
丸ごと埋めるので、立つビット数は**実 CPU 数ではなく常に 8200**
(`BITMAP_SIZE(8192) = 1025` バイト × 8) になる。1 サンプルで 8200 行ぶん進むため、
`sar -A` はパイプ出力でも 11 サンプルごとに見出しを出し直す。
ここを「選択した CPU の数」で代用すると、24 CPU のホストで 25 行しか進まず、
本家が 11 サンプルごとに出す見出しが 1 度も出なくなる
(実データ 29 本で、本家が生成したテキストと見出しの行番号が一致することを確認した)。

#### `text` の既定は要約 — 何を落としてよいか

規律を全部守った結果、1 エピソードが約 55 行になり、43 activity のホストでは
読み切れない量になった。**冗長さの正体は情報量ではなく繰り返しである。**
`text` の既定を要約にし、従来の全文は `--verbose` に移した
(`json` / `ndjson` は指定によらず全フィールドを出す。省くのは `text` だけ)。

**繰り返しは 2 つの軸で起きる。**

| 軸 | 実測 | 対処 |
|---|---|---|
| 1 エピソードの中 | 検出 1 件のエピソードで、見出し・「関わった系列」・「検出の根拠」が同じことを 3 回言う | 検出ごとの内訳を落とす |
| エピソードの間 | 同じ系列の変化が繰り返し起きると、同じ指標名・解釈・注意が並ぶ | **見出しの系列ごとにまとめる** |

エピソードは検出の**始まり**でまとめる (規律 19)。この定義は正しいが、
1 日を通じて断続的に鳴る系列では「始まり」がその回数だけできる。
1 件ずつ並べると確認すべき指標を探しにくいため、要約では
**系列を先に、時刻をその下に**置く。系列ごとの見出しに件数と最高優先度を出し、
各エピソードは「範囲・その回の優先度・採取回数」の 1 セルになる。

まとめても次は失わない。

- **いつ** — エピソードごとの範囲を時刻で並べる (打ち切ったら件数を出す)
- **同時に鳴った別の系列** — `+N 系列` として各セルに出す。
  複数系列が重なったエピソードは「何かが起きた」手がかりで、埋めてはいけない
- **充足度のばらつき** — 系列内で違えば「最低限〜十分 (件により違う)」と出す

`--verbose` は従来どおりエピソードを 1 件ずつ出す。

既定の要約は調査の入口として、**優先度順に最大5系列、各系列の時刻を最大3件**までにする。
時刻も優先度順に選び、後半の重要な検出を先頭の軽微な検出で押し出さない。
背景の所見は優先度順に最大3件。表示を省いた件数と全件を確認する方法を必ず示す。
検出自体・JSON・NDJSON には表示上限を適用しない。
代表例には観測値と判定理由を付け、優先度のラベルだけで判断させない。
系列のグループ化と順位付けは `analyze::assessment`、表示上限と書式は `output` が担当する。

表示上限による省略に加えて、次の繰り返しを省く。

| 落としてよいもの | 理由 |
|---|---|
| 検出パターンが決まれば中身も決まる固定文 | エピソード数に比例して同じ文が増える。「考えられる解釈」がこれ |
| 報告の別の場所が同じことを言っている文 | 比較基準の出所 (ヘッダと注意が言う)、`MAD = 0` の留保 (検出の説明文が言う) |

**落としてはいけないもの**は、規律をそのまま写したものになる。

- **比較基準の出所** (規律 3) — ヘッダと注意で 1 度ずつ言う。検出ごとに繰り返す必要はない
- **系列に固有の留保** (規律 3) — 中央値そのものが固定条件の内側、材料の半分以上が
  固定条件を満たす、要求当たりの平均。**一般論の注意書きでは代用できない**
- **評価の網羅度** (規律 7 / 14) — 評価できなかった系列と理由、構造的に見ていない範囲
- **確かめていないこと** (規律 21 / 22) — 観測と解釈の境界。要約では**指標ごとに 1 度**
  末尾へ集約する。位置を変えるだけで、落としてはいけない。
  `--verbose` や JSON への案内で代用すると、`text` 単体から境界が消える
- **省いたことの明示** — 何が出ていないか分からなければ、読み手は
  「この所見にはこれだけしか根拠が無い」と読む

固定条件の検出には比較基準の留保を出さない。**その経路は基準を判定に使っていない**ので、
使っていない根拠で所見を割り引くことになる (規律 17)。
...
系列の時刻が繋がらないことだけでは、ホスト全体の採取停止とは判定しない。
デバイスの消失・再出現でも穴ができるため、再起動や区間長0などの根拠が無ければ
不連続の影響はその系列に留める。
また、`--baseline-scope window` の瞬時値は採取時刻で選び、区間値は時間範囲の重なりで選ぶ。
報告範囲に epoch 秒と日内時刻が混在する場合は、epoch で切った残りに日内条件を適用する。
夏時間の切り替えでは、存在しない壁時計を補わず、重複する壁時計は両方評価する。
