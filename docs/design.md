...
| エピソードの間 | **1 日分の実ファイルで 111 エピソード / 23 系列。** `pgscand` が 18 件、`tps` が 15 件と、同じ系列が散発的に鳴るたびに別のエピソードになる | **見出しの系列ごとにまとめる** |

エピソードは検出の**始まり**でまとめる (規律 19)。この定義は正しいが、
1 日を通じて断続的に鳴る系列では「始まり」がその回数だけできる。
1 件ずつ並べると「何が鳴っているか」が 1400 行へ薄まるので、要約では
...
落としてよいのは次の 2 つだけで、どちらも**読み手が新しく得る情報が無い**もの。
