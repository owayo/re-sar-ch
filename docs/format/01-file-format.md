...
|---|---|---|---|
| `0x115a` / 4442 | 3.2.4 以前 | 〜2000-08-06 | 3.2.4 の `sa.h` を実測。1.0〜3.2.3 / 3.3.1 は未入手 (同値と推定) |
...
| magic | sysstat | magic の位置 | 先頭ヘッダのバイト数 | 状態 |
|---|---|---:|---:|---|
| `0x115a` | 〜3.2.4 | `file_hdr` @ **4** | (未実測) | 識別のみ |
| `0x215a` | 3.3.2 | @ **4** | (未実測) | 識別のみ |
| `0x215b` | 3.3.3〜3.3.5 | @ **4** | (未実測) | 識別のみ |
| `0x215d` | 3.3.6〜4.0.7 | @ **4** | 229 | 識別のみ |
| `0x2160` | 4.1.4 | @ **4** | (未実測) | 識別のみ |
| `0x2163` | 4.1.7〜5.0.6 | @ **4** | 240 | 識別のみ |
| `0x2165`〜`0x216c` | 5.1.3〜7.1.5 | @ **36** | 240 | 識別のみ |
| `0x216d` / `0x216e` | 7.1.6〜8.0.4 | @ **32** | 240 | 識別のみ |
| `0x216f` | 8.1.1〜8.1.2 | `file_magic` @ 2 | 8 + 304 | 識別のみ |

> **`0x216f` は中間世代**である。`file_magic` が頭に付いた最初の版だが、
> **本体はまだ旧形式**で、`file_activity[]` への転換は次の `0x2170` から。
> 「先頭が `0xd596` なら現行形式」という一般化はここで破れる。

未入手の版 (`0x215e` / `0x215f` / `0x2161` / `0x2162` / `0x2164`) は**登録しない**。
隣接世代から推定して読むより、「知らない」と言う方が安全である。
...
**識別と診断まで。ヘッダ解釈・レコード境界・統計デコードは未実装。**
`Error::UnreadableGeneration` として「どの世代のファイルか」を明示して拒否する。
「壊れている」「sysstat のファイルではない」と混同して報告しないために、
エラー種別を分けてある。
