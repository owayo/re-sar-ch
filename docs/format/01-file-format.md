...
### 2.3 FORMAT_MAGIC (pre-git 時代 = CHANGES の `[0xNNNN]` 表記より)

CHANGES には各フォーマット変更時に `WARNING: Daily data files format has changed ... [0xNNNN]` と
magic が明記されている。

| 値 (16進 / 10進) | 導入バージョン | リリース日 |
|---|---|---|
| `0x215d` / 8541 | 3.3.6 | 2001-03-04 |
...
| `0x2170` / `0x7021` 以下 | 読めない。本家 `sadf -c` も非対応。「変換を試せ」というヒントも出さない (`fmt_magic >= 0x2171` でゲート) |
...
> 変換時には**変換を実行したマシンの HZ** (`sysconf(_SC_CLK_TCK)`) が `sa_hz` に書かれ、
> さらにレコードヘッダの `uptime_cs` の計算にも使われる (§5.6)。
> 元ファイルを作ったマシンと HZ が異なる場合、**uptime が誤変換される**。
> 回避策は `sadf -c -O hz=<値>` で元マシンの HZ を明示指定すること
> (このオプションは **sysstat 12.3.3 で追加**)。指定値は `sa_hz` と `uptime_cs` の両方に効く。
...
| 19 | G0 (`0x2170`) / `0x2169` 以前は本家も読めない | 対応しない旨をエラーメッセージで明示する |
