...
全 37 activity の fixture を作り、式から手計算した値で el7 の癖を固定する
(CPU の集約スロット・オフライン・tickless・ゲスト時間、平均の整数除算と偶数丸め、
ページサイズ、NIC とディスクの再登録と枠の再利用、USB / FS の要約リスト、
ビットマップによる行の選択、RESTART / COM 行、見出しの再表示、`-s` / `-e` / `-i` / count、
ローカル時刻、引数とファイルの誤り)。`src/series/el7.rs` の単体テストは、同じ値の組で
関数単位の癖を固定する。

§6.2 のオラクルを `RESARCH_EL7_ORACLE` に与えると、各テストが同じ fixture・引数・
`LC_ALL=C`・`TZ` で本家も実行し、stdout と終了コードの一致まで確かめる
(`RESARCH_EL7_FIXTURE_DIR` を与えると fixture をそのディレクトリへ残す)。

```bash
RESARCH_EL7_ORACLE=<ビルドした el7 の sar> cargo test --test sar_el7
```

ページサイズを 4096 以外にした `-R` と、誤りの終了コード (§7) だけは本家と比べない。
...
- 誤りの終了コードは一律 1。本家は開けないファイルで 2、読めないファイル
  (世代違い・短すぎる・読める `A_CPU` が無い) で 3 を返す
  (選んだ activity が無いときと usage は本家も 1)
