...
旧世代のヘッダは HZ を持たない。本家の `sadf -c` は**変換を実行したマシンの HZ** を
書くので同じ入力でも実行環境で出力が変わるが、`resarch` はファイル自身の `uptime`
カウンタから推定する (再起動をまたぐ対は使わない)。採用した値と出所は必ず報告される。
...
| 独自出力に `A_IRQ` の CPU 別内訳が無い | 行 = 割り込み、値 = CPU `all` の代表値。CPU 別が必要なら `sadf -j` / `-x` を使う |
| `sadf -g` / `-l` が動かない | SVG / PCP は未実装。実行時に理由つきで拒否する |
