...
既定の `text` は**エピソードを見出しの系列ごとにまとめる**。同じ系列が 1 日を通じて
散発的に鳴るとその回数だけエピソードができるためで、実測では 23 系列に対して
111 エピソードが立った。系列ごとに「指標名・件数・最高優先度」を出し、その下に
各エピソードの時刻・優先度・採取回数を並べる (8 件を超えたら件数を出して打ち切る)。
**同時に別の系列が鳴ったエピソードには `+N 系列`** が付く — 重なりは
「何かが起きた」手がかりなので、ここを読み飛ばさないこと。

検出ごとの内訳 (観測値の列挙・比較基準の数値・窓の幅) と「考えられる解釈」は省く。
...
- **評価の網羅度** — 評価できなかった系列とその理由
- 系列に固有の留保 — `! 比較基準が異変側へ寄っている疑いがある` 等
- **確かめていないこと** — エピソード本文ではなく末尾に、**指標ごとに 1 度**まとまる

`text` でエピソードを 1 件ずつ・検出ごとの数値まで追いたいときだけ `--verbose` を付ける
(出力は要約導入前と 1 文字も変わらない)。
