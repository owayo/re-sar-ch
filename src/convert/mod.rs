...
//! | 既知 id で未知の magic | 中断しない。magic が現行より大きければ現行形式とみなして item の先頭 `fsize` バイトを写し、小さければ旧形式のどれかとみなして変換する (12.8.0 の `sadf -c` で確認) | 旧バイト列を**素通し**する (`file_activity` も旧申告値のまま) | 意味の分からないバイト列をフィールド単位で動かすと、別の意味の値が統計として出る |
...
//! | `record_header` の時刻 | ホストとバイト順が逆のファイルでは `upgrade_record_header()` が `ust_time` (32bit ライタのファイルでは `uptime_cs` も) を壊す | 値として読んでから書くので壊れない | 同上 (12.8.0 の `sadf -c` で確認) |
...
pub fn convert(
    ...
    if !report.opaque_activities.is_empty() {
        let names: Vec<String> = report
            ...
            .collect();
        // 本家が中断するのは未知 ID だけ。既知 ID の未知 magic は既知の形式とみなして
        // 変換を続ける (12.8.0 の sadf -c で確認) ので、両者を分けて書く。
        report.warnings.push(format!(
            "構造を解釈できない activity ({}) は旧バイト列のまま書き出した \
             (本家 sadf -c は未知の ID なら exit 1 で中断し、既知の ID なら magic が未知でも \
             既知の形式とみなして書き換える)",
            names.join(", ")
        ));
    }
    ...
    for slice in rec.slices {
        ...
        if plan.has_nr {
            if out_count == 0 {
                ...
                // 本家 `sar` も reSARch の読み取り側もそのまま読める。
                // 元ファイルでは全枠が空だったことを、変換の注意として残す。
                report.warn_once(format!(
                    "{}: 番兵の手前に有効な item が 1 つも無いレコードがあり、\
                     レコード内件数 0 を書いた (現行形式として正当な件数で、そのまま読める)",
